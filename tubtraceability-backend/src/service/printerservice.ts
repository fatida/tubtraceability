...
export function formatPrintCommand(printData: IPrintData): Promise<{ inkjet: string, label: string }> {
    return new Promise((resolve, reject) => {
        try {
            let originalDateString = printData.date

            // Validation for empty or invalid date format
            if (!originalDateString || !/^\d{2}\.\d{2}\.\d{4} \d{2}:\d{2}:\d{2}$/.test(originalDateString)) {

                const now = new Date();
                originalDateString = now.toLocaleDateString('tr-TR', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
